...
| `ww-opening-gap.ts` | Without any marks: how long after a battle opens on chain the first trade lands, **who** lands it, and whether their side won. An **upper bound** on the fastest participant's knowledge, not the announcement lag. Counts the platform's own wallet separately from traders |
