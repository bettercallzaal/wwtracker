...
## These are NOT the 47, and the difference decides how they are archived

The 47 branches retired on 2026-09-19 were provably lossless in the strongest
sense: **every tip was an ancestor of `main`**, so deleting the name removed a
pointer and nothing else. Every commit stayed reachable.

**None of these 121 are.** Measured: **0 of 121 tips are ancestors of `main`.**
That is what squash-merging does - the branch's CONTENT reaches `main` as a new
single commit while its original commits never become ancestors. Delete the
name and those commits become unreachable, and an unreachable object is
eventually garbage-collected.

**The work is not at risk either way** - every one of the 121 has its content on
`main`, which is the test above. What would go is the HISTORY: the individual
commits, their messages, the shape of how the work was done. For the 54 merged
ones GitHub also keeps the pull request page and its diff regardless.

**So the history is archived first, as a file in this repo.**
`archive/branches-121-2026-09-20.bundle`, 1.8 MB, holding all 121 branches with
complete history. Recovery no longer depends on GitHub's garbage collector or on
any one laptop's clone.

Restore all of them into an empty repository:

```
git fetch archive/branches-121-2026-09-20.bundle 'refs/archive/121/*:refs/heads/*'
```

**Verified, not assumed.** Fetched into a fresh `git init` and got 121 branches
back, tip shas matching `origin`, files intact. A first attempt at this bundle
recorded `refs/remotes/*` names, which `git clone` ignores, and restored **zero**
branches while a check printed success anyway - the check was an unconditional
`echo`. Both were fixed and the verification now asserts the count and compares
a sha.
